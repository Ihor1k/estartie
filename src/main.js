...
const cards = document.querySelectorAll(".method-card");
  ...
  const triggers = document.querySelectorAll(".method-trigger");
  ...
  function setActive(step) {
    ...
    const percent = ((step - 1) / 4) * 100; // 0 → 100%
    fill.style.setProperty("--progress", percent + "%");
  }

  // при кліку по цифрі — скролимо до відповідного тригера
  dots.forEach((dot) => {
    dot.addEventListener("click", () => {
      const step = dot.dataset.step;
      const trigger = document.querySelector(
        `.method-trigger[data-step="${step}"]`
      );
      if (trigger) {
        window.scrollTo({
  ...
  const observer = new IntersectionObserver(
    ...
    {
      threshold: 0.55, // більше половини тригера в вʼюпорті
    }
  );
